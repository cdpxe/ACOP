...
The carrier-symbol distribution $p_i$ is based on the selected ToS values from the [MAWI](https://mawi.wide.ad.jp/mawi/) trace used in the example. The most frequent ToS value (0x00) is intentionally excluded, as stated in the article. The next 4 (2nd to 5th most frequent) values are normalized over the four carrier symbols considered in the calculation. Thus, the $p_i$ values in the spreadsheet should be read as the carrier distribution after this selection step, i.e., as the probabilities of the extracted carrier symbols, not as frequencies over all observed ToS values.
