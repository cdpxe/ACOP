...
This spreadsheet `IWAPS-Example-Textdata` accompanies the numerical example in Section 4 of our paper.
...
where L_bar is the average number of 2-bit carrier symbols per original source symbol, q_i is the expected distribution of encoded message symbols, and p_i is the selected carrier-symbol distribution. The result is an expected packet count per original message symbol, not per encoded 2-bit symbol.
